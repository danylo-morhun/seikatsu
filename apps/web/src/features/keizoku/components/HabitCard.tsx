		...
		<div className="flex flex-col gap-2 rounded-lg border border-border/50 bg-card px-3 py-2.5">
			<div className="flex items-center gap-3">
				<button
					...
					disabled={pending}
					className={cn(
						"flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-lg transition-colors",
				...
				<Link href={`/keizoku/${habit.id}`} className="min-w-0 flex-1">
					...
					{streak > 0 && (
						<p className="mt-0.5 flex items-center gap-1 text-xs font-medium text-orange-400">
							<HugeiconsIcon icon={FireIcon} className="h-3.5 w-3.5" />
							{streak}
						</p>
					)}
				</Link>
			...
			<div className="flex items-center justify-end gap-3 border-t border-border/40 pt-1.5">
				<button
					type="button"
