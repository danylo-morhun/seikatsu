...
export function IncomeChart({ balances, currency }: Props) {
	const sources = balances.filter((b) => b.type === "INCOME" && Number.parseFloat(b.balance) > 0);

	const data = sources.map((b, i) => ({
		name: b.name,
